...
sec.execute("CREATE TABLE maeling ( ID SERIAL PRIMARY KEY, orku_ID INT, sendandi_maelingar VARCHAR NOT NULL, tegund_maelingar VARCHAR NOT NULL, FOREIGN KEY (orku_ID) REFERENCES orku(ID));")
...
with fyrstdb:
    ...
    for i in cur:
        ...
        with seconddb:
            sec.execute("INSERT INTO maeling (orku_ID, sendandi_maelingar, tegund_maelingar)"
                "VALUES ((SELECT ID FROM orku WHERE stadur = %s), %s, %s);", 
                (i[1], i[3], i[2]))
            if i[6] is not None:
                ...
                sec.execute("INSERT INTO gildin (maeling_ID, gildi_kwh, timi)"
                        "VALUES ("
                        "(SELECT ID FROM maeling WHERE ID = %s), %s);", 
                        (i[0], i[5]))
...
cur = fyrstdb.cursor()
cur.execute("SELECT * FROM raforka_legacy.orku_einingar LIMIT 100;")
