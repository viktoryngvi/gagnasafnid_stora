...
sec.execute("CREATE TABLE orku_stodvar_eigandi (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, heiti_eigandans VARCHAR UNIQUE);")
sec.execute("CREATE TABLE hnit (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, X_HNIT DOUBLE PRECISION, Y_HNIT DOUBLE PRECISION);")
sec.execute("CREATE TABLE orku (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, stadur VARCHAR NOT NULL UNIQUE, dagsetning_uppsett DATE NOT NULL, hnit_id INT NOT NULL, stodvar_id INT NOT NULL, FOREIGN KEY(hnit_id) REFERENCES hnit(ID), FOREIGN KEY(stodvar_id) REFERENCES orku_stodvar_eigandi(ID));")
...
sec.execute("CREATE TABLE notandi (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, hnit_id INT, stadsetning VARCHAR NOT NULL, kennitala VARCHAR NOT NULL UNIQUE, eigandi VARCHAR NOT NULL, ar_stofnad VARCHAR NOT NULL, FOREIGN KEY(hnit_id) REFERENCES hnit(ID));")
sec.execute("CREATE TABLE maeling(ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, orku_ID INT, sendandi_orku_id INT, sendandi_eigandi_id INT, tegund_maelingar VARCHAR NOT NULL, FOREIGN KEY (orku_ID) REFERENCES orku(ID), FOREIGN KEY (sendandi_orku_id) REFERENCES orku(ID), FOREIGN KEY (sendandi_eigandi_id) REFERENCES orku_stodvar_eigandi(ID), sendandi VARCHAR GENERATED ALWAYS AS ((CASE WHEN sendandi_orku_id IS NOT NULL THEN 's' || sendandi_orku_id WHEN sendandi_eigandi_id IS NOT NULL THEN 'e' || sendandi_eigandi_id END)) STORED, UNIQUE (orku_ID, sendandi, tegund_maelingar), CONSTRAINT exclusive_sender CHECK ((sendandi_orku_id IS NOT NULL AND sendandi_eigandi_id IS NULL) OR (sendandi_orku_id IS NULL AND sendandi_eigandi_id IS NOT NULL)));")
sec.execute("CREATE TABLE gildin(ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, maeling_ID INT, notandi_ID INT NULL, gildi_kwh NUMERIC NOT NULL, timi TIMESTAMP NOT NULL, FOREIGN KEY (notandi_ID) REFERENCES notandi(ID), FOREIGN KEY (maeling_ID) REFERENCES maeling(ID));")
...
with seconddb:
    for i in cur:
        ...
        sec.execute("INSERT INTO gildin (ID, maeling_ID, gildi_kwh, notandi_ID, timi) VALUES ("
            "%s, "
            ...
            (i[0], i[1], i[3], i[3], i[2], i[5], i[6], i[4]))
